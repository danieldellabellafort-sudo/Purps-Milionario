...
import { doc, setDoc, onSnapshot, collection, query, getDocs, deleteDoc, writeBatch } from "firebase/firestore";
...
const Index = () => {
  ...
  const resetAll = async () => {
    if (!window.confirm("⚠️ Tem certeza? Isso vai APAGAR todas as contas, histórico e dados do Firebase. Essa ação é irreversível!")) return;
    if (!window.confirm("🚨 CONFIRMAÇÃO FINAL: Apagar TUDO mesmo?")) return;
    try {
      // Apaga dados históricos
      await deleteDoc(doc(db, "app", "global_data_v2"));

      // Apaga todos os usuários
      const usersSnap = await getDocs(collection(db, "diary_users"));
      const batch = writeBatch(db);
      usersSnap.forEach(d => batch.delete(d.ref));

      // Apaga fotos de perfil (chunks)
      const picsSnap = await getDocs(collection(db, "profile_pics_chunked"));
      picsSnap.forEach(d => batch.delete(d.ref));

      // Apaga imagens de entradas
      const imgSnap = await getDocs(collection(db, "entry_images"));
      ...
      toast.error(`Erro ao resetar: ${e.message || "desconhecido"}`);
    }
  };
        ...
        <div className="max-w-5xl mx-auto px-4 py-4 flex flex-col sm:flex-row items-center justify-between gap-3">
          ...
          <div className="flex items-center gap-2 sm:gap-4">
            ...
            <div className="flex items-center gap-1 border-l pl-2 sm:pl-4 border-muted-foreground/20">
              ...
              {user === "MASTER" && (
                <Button variant="ghost" size="icon" onClick={resetAll} title="🗑️ Resetar TUDO (Admin)">
                  <span className="text-red-500 font-bold text-xs">RESET</span>
                </Button>
              )}
          ...
          {top20Activities.length > 0 ? (
            <div className="flex flex-col gap-3">
             ...
             <div className="flex flex-col items-center justify-center py-10 text-muted-foreground bg-accent/20 rounded-xl p-6 border-dashed border-2">
               ...
               <p className="font-medium text-foreground text-center mb-1">Este registro ainda está sem movimentações.</p>
